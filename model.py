...
class YoloLayer(Layer):
    ...
    def call(self, x):
        ...
        class_mask = tf.expand_dims(
            tf.cast(tf.equal(tf.argmax(input=pred_box_class, axis=-1), true_box_class), tf.float32),
            4)
        recall50 = tf.reduce_sum(input_tensor=tf.cast(iou_scores >= 0.5, tf.float32) * detect_mask * class_mask) / (
                count + 1e-3)
        recall75 = tf.reduce_sum(input_tensor=tf.cast(iou_scores >= 0.75, tf.float32) * detect_mask * class_mask) / (
                count + 1e-3)
        ...
        true_box_xy, true_box_wh, xywh_mask = tf.cond(pred=tf.less(batch_seen, self.warmup_batches + 1),
                                                      # true_box_xy, true_box_wh, xywh_mask = tf.cond(batch_seen,
                                                      true_fn=lambda: [true_box_xy + (
